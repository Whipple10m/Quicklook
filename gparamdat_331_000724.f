...
*
* This one line for Y2K compliency. What's all the fuss about ? SJF 990831
*
...
3009       continue
           pairexist = .false.
           write(*,*)'PD ** WARNING! Cannot find corresponding pair'
           write(*,*)'PD ** file. Tubes turned off may not match.'
           write(infunit,*)
     &      'PD ** WARNING! Cannot find corresponding pair'
           write(infunit,*)
     &      'PD ** file. Tubes turned off may not match.'
           if (pad) then
             write(*,*) 'PD ** aborting gparamdat.'
...
C     &                   ped_npmt,ped_mode,ped_mpmt)  JQ 961227
     ...
     &   ped_file(1:lnblnk(ped_file))
      ...
      write(*,*)' PD ** Pedestals:'
      write(6,895)(peds(i),i=1,npmt)
      write(*,*)' PD ** Pedestal variances:'
      write(6,895)(pedvar(i),i=1,npmt)
*
...
C     &           ped_eventcnt,ped_npmt,ped_mode,ped_mpmt) JQ 961227
     ...
     +    '  PD ** Read pedestals from other file in padding pair:',prid
         write(*,*)' PD ** Pedestals:'
         write(6,895)(prpeds(i),i=1,npmt)
         write(*,*)' PD ** Pedestal variances:'
         write(6,895)(prpedvar(i),i=1,npmt)
*
         write(infunit,861),
     +    '  PD ** Read pedestals from other file in padding pair:',prid
         write(infunit,*)' PD ** Pedestals:'
...
*
	 ...
	 if (status.gt.0) then
	    write(*,*)' PD ** Can not read nitrogen gains from file:',
	 ...
	 write(*,*)' PD ** Gains: '
	 write(6,895)(gains(i),i=1,npmt)
	 write(*,*)' PD ** Gain variances: '
	 write(6,895)(gainvar(i),i=1,npmt)
*
...
*
                if(abs(diff_sid_start-trk_ra_offset).gt.0.1) then
                  write(6,
     &             '(''  PD ** WARNING: Sidereal time missmatch! '')')
                  write(infunit,
     &             '(''  PD ** WARNING: Sidereal time missmatch! '')')
                endif
              endif
...
*
      ...
      write(6,'(''  PD ** Duration of '',a6,''           : '',f9.3)')
     &  filename(1:6),duration 
      if(prduration.lt.10000.0) then
       write(6,'(''  PD ** Duration of pair file '',a6,'' : '',f9.3)')
     &  prname(1:6),prduration
      endif
      write(6,'(''  PD ** Cutting to '',f9.3,'' sec '')')
     &  ltimehi
*
      write(infunit,'(''  PD ** Duration of '',a6,''           : '',
     &  f9.3)')filename(1:6),duration 
      if(prduration.lt.10000.0) then
       write(infunit,'(''  PD ** Duration of pair file '',a6,'' : '',
     &  f9.3)')prname(1:6),prduration
      endif
      write(infunit,'(''  PD ** Cutting to '',f9.3,'' sec '')')
     &  ltimehi
*
...
*
     ...
     &    ''   DEC (deg):'',f8.4)')dec,decdeg
        write(infunit,'(''        MJD           :'',f9.1)')mjd+frjd
*
...
*
     ...
     &         f8.4)')xoff,yoff
        write(*,*)'elevation',alt*convd
              write(6,'(''  PD ** First utdec (dec.hr)  :'',f8.4,
     &         /,''        Sidereal time (hr)    :'',f8.4,
     &         /,''        Derotation angle (deg):'',f9.4,
     &         /,''        Telescope AZ (deg)    :'',f9.4,
     &         /,''        Telescope EL (deg)    :'',f8.4)')
     &         utdec,stime,theta*convd,az*convd,alt*convd
*
	      write(infunit,'(''  PD ** xoffset,yoffset (deg) :'',f8.4,
     &         f8.4)')xoff,yoff
              write(infunit,
     &         '(''  PD ** First utdec (dec.hr)  :'',f8.4,
     &         /,''        Sidereal time (hr)    :'',f8.4,
     &         /,''        Derotation angle (deg):'',f9.4,
     &         /,''        Telescope AZ (deg)    :'',f9.4,
     &         /,''        Telescope EL (deg)    :'',f8.4)')
...
*-- END MAIN LOOP
*----------------
       write(6,'(''  PD ** Last utdec (dec.hr)   :'',f8.4,
     & /,''        Sidereal time (hr)    :'',f8.4,
     & /,''        Derotation angle (deg):'',f9.4,
     & /,''        Telescope AZ (deg)    :'',f9.4,
     ...
     & utdec,stime,theta*convd,az*convd,alt*convd
*
       write(infunit,'(''  PD ** Last utdec (dec.hr)   :'',f8.4,
     & /,''        Sidereal time (hr)    :'',f8.4,
     & /,''        Derotation angle (deg):'',f9.4,
     & /,''        Telescope AZ (deg)    :'',f9.4,
     & /,''        Telescope EL (deg)    :'',f9.4)')
...
*     endif
*
      write(6,'(''  PD ** Sidereal duration of '',a6,'': '',
     &  f8.3)')filename,
     +   (time-first7)*366.2422/(60.0*365.2422)
      write(6,'(''        Livetime (UT sec) : '',f8.3)'),ltime
*
      write(infunit,'(''  PD ** Sidereal duration of '',a6,'': '',
     &  f8.3)')filename,
     &    (time-first7)*366.2422/(60.0*365.2422)
      write(infunit,
     &   '(''        Livetime (UT sec) : '',f8.3)'),ltime
      
      write(6,'(''        Number of Events  : '',i)'),nevents
      write(6,'(''        Number Muon Rings : '',i)'),nring
      write(infunit,'(''        Number of Events  : '',i)'),nevents
      write(infunit,'(''        Number Muon Rings : '',i)'),nring
*
...
160   format(' PD ** Cannot find n2 gains for given date/id: ',i6,1x,i4)
...
867   format('  PD ** Picture threshold :',f5.2,' Boundary threshold :',
     & f5.2)
890   format('GPARAMDAT ** File: ',a12,1x,'Date: ',i6,1x,'N2 ID: ',i6)
895   format(10f7.3)
...
*
     ...
     &                    pmtmask(ixmax,iymax+1,i)
          ...
          do i=1,npict
            ...
            m=(k-1)/32
*
* If a tube has contributed to the arc
* include it's pulseheight in the sum
*
...
*
* Added 961104 by JB - Calculate amount of light in an anulus about
