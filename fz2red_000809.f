...
*     which uses Fortran90.  First step: replace "inlude gdf.inc" calls
...
* ** It also contains
* ** the latched oscillator value (gate_close), the oscillator value of
* ** the last GPS second marker from the 10m, the event
* ** number, and the live time counter.  I have seen some events (at the
* ** beginning of gd000230.fz, for example) with high counter values
* ** before the counter is reset to 1.  Skip all events until the
* ** first event is seen.
*
...
*
      ...
      runid_exists = .false.
*
* Fix the header by promting for all header information if the
...
*
      if(ffixhdr) then
          ...
          dec_rad = rdec
*
* Convert back using same code for rad to hhmmss etc as
* a check
*
...
* lastgclose<0.  I believe this possiblitiy was overlooked, although
...
* Check that first live time is not very large, and if it is put in 
* an offset.
...
* as a sort of fix, don't include the seconds in the live time calculation.
     ...
     >                          gdf_ev10%live_sec
                           ...
                           type*,'Subtracting from the live time ',
     >                          gdf_ev10%live_sec,' sec'
                        endif
                        livesec_off=livesec_old
                        livens_off=livens_old
...
* Weird
     ...
     >                    dfloat(livens_old)/1.d9).gt.1.0d0) then
                        type*,'***** WARNING *****'
                        type*,'Live time jumped forward by more than'
     ...
     >              (gdf_ev10%live_sec+gdf_ev10%live_ns).gt.0) then
                  ...
                  if (lvwarns.eq.5) then
                     type*,'****************************************'
                     type*,'No more warnings will be printed'
                     type*,'****************************************'
                  endif
               endif
* MAC 970912
...
* Weird
     ...
     >                    *defosc_freq/elosc_freq).gt.1.0d0) then
                        type*,' ***** WARNING *****'
                        type*,' Elapsed time jumped forward more than ',
     +                  '1 second - may be inaccurate.'
*                        type*,' *******************'
                     endif
                  endif  
...
* in that case use elapsed time from the elapsed time scaler.
            ...
            if ((timtick.eq.0.).or.(.not.usetmtick)) then
               if (usetmtick) then
                  type*,' FZ *****WARNING*****'
                  type*,' Elapsed time from gate close'
                  type*,' is 0, using elapsed time scaler'
                  usetmtick=.false.
               endif
...
*    &  ra_off2
     ...
     &    .gt. 0.05) then
        if(idate.lt.950629) then
         ...
         write(u,*)
     &     '    ** Seting the RA offset to the nominal value: 30min'
         ra_offset = 30.0
        else
...
*      write(u,'('' VAX time                 : '',f11.4)')gdf_run%itime
       ...
       if (defdate.lt.970901) then
          write(u,'('' Mean GPS-osc. dev [msec] : '',f10.3)')
     >         meantdev*1000.0
          write(u,'('' RMS GPS-osc. dev [msec]  : '',f10.3)')
     >         sigtdev*1000.0 
     ...
     >         e14.8)') elosc_freq
          write(u,'('' Ave. Est. 10 MHz osc. freq. [Hz]: '',e14.8)')
     >         ave_osc
          write(u,'('' Std. dev. of est. freq. [Hz]: '',e14.8)')
     >         dev_osc
...
* is missing.
     ...
     &  f8.3)')dduration
       write(u,'('' Run duration (elapsed GPS time [sec])    : '',
...
*
     ...
     &  status='unknown')
      ...
      write(hunit,'(''gps_sid_start  '',f7.4)')stime
...
*     write(hunit,'(''nom_utc_end    '',f12.5)'),gdf_run%utc_end
      write(hunit,'(''first_sid_min  '',i4)')ist
      write(hunit,'(''first_ut_min   '',i4)')iut
...
*     write(hunit,'(''first_evt_utc  '',f11.4)'),firstgpsutc
      write(hunit,'(''n_sec_marks    '',i4)'),secmarks
...
140   format(' File ',a,' does not exist.  Exiting...')
 ...
 153  format(' FZ ** GRS bits 21-24 of set 3 are non-zero ',
     >     i9,' times.  Call Purdue in the morning.')
 154  format(' FZ ** Quicklook GRS - DACQ GRS >10 usec ',
     >     i9,' times.  Check bit conversions and MJD.')
 ...
 180  format(' FZ ** ',a,' bits set ',i9,' times.  This should',
     >     ' never happen.')
