  ...
  for(i=0;i<490;i++)
    {
      int j;
      int k;
      wxdeg[i]=WC490Xcoord[i];
      wydeg[i]=WC490Ycoord[i];
      wradius[i]=WC490Radius[i];
      for(j=0;j<7;j++)
	{
	  k=WC490Neighbors[i][j]+1;
	  if(k==0)k=491;
	  nbr_list[i*7+j]=k;
	}
    }
...

{
  ...
  while (((lock_fp=fopen(lock_file,"r"))!=NULL) && (wait<120)){
    fclose(lock_fp);
    printf(" read_peds_: /tmp/cpeds.lock exists, trying again in 20 s\n");
           ...
           2 error (lock file)  */
{
  ...
  while (((lock_fp=fopen(lock_file,"r"))!=NULL) && (wait<120)){
    fclose(lock_fp);
    printf(" chk_peds_: /tmp/cpeds.lock exists, trying again in 20 s\n");
    sprintf(sys_cmd,"sleep %d",20);
    system(sys_cmd);
    wait+=20;
  }
...

{
  ...
  while (((lock_fp=fopen(lock_file,"r"))!=NULL) && (wait<120)){
    fclose(lock_fp);
    printf(" write_peds_: /tmp/cpeds.lock exists, trying again in 20 s\n");
    sprintf(sys_cmd,"sleep %d",20);
    system(sys_cmd);
    wait+=20;
  }
...
/* returns: 0 success, 1 cannot find entries, 2 error (lock file)  */
{
  ...
  while (((lock_fp=fopen(lock_file,"r"))!=NULL) && (wait<120)){
    fclose(lock_fp);
    printf(" del_peds_: /tmp/cpeds.lock exists, trying again in 20 s\n");
    sprintf(sys_cmd,"sleep %d",20);
    system(sys_cmd);
    wait+=20;
  }
...

{
  ...
  while (((lock_fp=fopen(lock_file,"r"))!=NULL) && (wait<120)){
    fclose(lock_fp);
    printf(" read_n2gains_: %s exists, trying again in 20 s\n",lock_file);
    sprintf(sys_cmd,"sleep %d",20);
    system(sys_cmd);
    wait+=20;
  }
  ...
  while(fgets(line, 90, fp) != NULL){
	...
	fscanf(fp,"%f",&gainvars[i]);
      ...
      n_lines=2*((n_entries+entries_per_line-1)/entries_per_line);
      for(i=0;i < n_lines;i++)
	fgets(line,90,fp);
    }
  }
           ...
           2 error (lock file)  */
{
  ...
  while (((lock_fp=fopen(lock_file,"r"))!=NULL) && (wait<120)){
    fclose(lock_fp);
    printf(" chk_n2gains_: %s exists, trying again in 20 s\n",lock_file);
    sprintf(sys_cmd,"sleep %d",20);
    system(sys_cmd);
    wait+=20;
  }
...
/* returns: 0 success, 1 cannot find entries, 2 error (lock file)  */
{
  ...
  while (((lock_fp=fopen(lock_file,"r"))!=NULL) && (wait<120)){
    fclose(lock_fp);
    printf(" del_n2gains_: %s exists, trying again in 20 s\n",lock_file);
    sprintf(sys_cmd,"sleep %d",20);
    system(sys_cmd);
    wait+=20;
  }
...

{
  ...
  while (((lock_fp=fopen(lock_file,"r"))!=NULL) && (wait<120)){
    fclose(lock_fp);
    printf(" chk_toff_: %s exists, trying again in 20 s\n",lock_file);
    sprintf(sys_cmd,"sleep %d",20);
    system(sys_cmd);
    wait+=20;
  }
...

{
  ...
  while (((lock_fp=fopen(lock_file,"r"))!=NULL) && (wait<120)){
    fclose(lock_fp);
    printf("\n read_toff_: %s exists, trying again in 20 s\n",lock_file);
    sprintf(sys_cmd,"sleep %d",20);
    system(sys_cmd);
    wait+=20;
  }
  ...
  if((fp = fopen(db_file, "r")) == NULL){
    printf("\n read_toff_ error: cannot find %s, exiting....\n",db_file);
    exit(2);
  }
  ...
  while(fgets(line, 2500, fp) != NULL){
	...
	sscanf(line,"%d%n",&tube,&nskip);
      ...
      printf("\n read_toff_: read entry for %s in %s\n",run_id,db_file);
      fclose(fp);
      return 0;
    } 
  }
  printf("\n read_toff_: entry does not exist for %s in %s\n",
	 run_id,db_file);
  fclose(fp);
  return 1;
}
...
/* returns: 0 success, 1 cannot find entries, 2 error (lock file)  */
{
  ...
  while (((lock_fp=fopen(lock_file,"r"))!=NULL) && (wait<120)){
    fclose(lock_fp);
    printf(" del_toff_: %s exists, trying again in 20 s\n",lock_file);
    sprintf(sys_cmd,"sleep %d",20);
    system(sys_cmd);
    wait+=20;
  }
